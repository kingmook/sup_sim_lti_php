<?php // vim: foldmethod=marker $OAuth_last_computed_siguature = false; 
...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod {
  ...
  public function build_signature($request, $consumer, $token) {
    ...
    $computed_signature = base64_encode(hash_hmac('sha1', $base_string, 
$key, true));
...
"443" &&
    ...
    // We weren't handed any parameters, so let's find the ones relevant 
to
    // this request.
  ...
  private function check_signature(&$request, $consumer, $token) {
    ...
    if (!$valid_sig) {
      ...
      if ( $OAuth_last_computed_signature ) {
          $ex_text = $ex_text . " ours= $OAuth_last_computed_signature 
yours=$signature";
      }
